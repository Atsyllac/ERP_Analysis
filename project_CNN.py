...
npz = np.load("C:\\Users\\cally\\ProgramsAndProjects\\NN Hwk\\Project\\thealldata.npz")
...
eeg_train_norm = (eeg_train - np.mean(eeg_train, axis=2, keepdims=True)) / (np.std(eeg_train, axis=2, keepdims=True) + 1e-9)
eeg_test_norm  = (eeg_test  - np.mean(eeg_test,  axis=2, keepdims=True)) / (np.std(eeg_test,  axis=2, keepdims=True) + 1e-9)
